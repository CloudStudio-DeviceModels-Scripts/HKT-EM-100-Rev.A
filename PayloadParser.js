// Test Payload: 1 2 21 01 28 13 08 00 01 02 03 04 31 75 00 00 02 23 02 01 22 00 00 00 00 00
...
function parseUplink(device, payload) {
    ...
    if (decoded.Total != null) {
        ...
        if (sensor1 != null)
            sensor1.updateGenericSensorStatus(decoded.Total);
    };
    ...
    if (decoded.Amount != null) {
        ...
        if (sensor2 != null)
            sensor2.updateGenericSensorStatus(decoded.Amount);
    };

    // Store A Phase Voltage
    if (decoded.A_phase_voltage != null) {
        var sensor3 = device.endpoints.byAddress("3");

        if (sensor3 != null)
            sensor3.updateVoltageSensorStatus(decoded.A_phase_voltage);
    };

    // Store B Phase Voltage
    if (decoded.B_phase_voltage != null) {
        var sensor4 = device.endpoints.byAddress("4");

        if (sensor4 != null)
            sensor4.updateVoltageSensorStatus(decoded.B_phase_voltage);
    };

 // Store C Phase Voltage
    if (decoded.C_phase_voltage != null) {
        var sensor5 = device.endpoints.byAddress("5");

        if (sensor5 != null)
            sensor5.updateVoltageSensorStatus(decoded.C_phase_voltage);
    };
 // Store A Phase Current
    if (decoded.A_phase_current != null) {
        var sensor6 = device.endpoints.byAddress("6");

        if (sensor6 != null)
            sensor6.updateCurrentSensorStatus(decoded.A_phase_current);
    };

 // Store B Phase Current
    if (decoded.B_phase_current != null) {
        var sensor7 = device.endpoints.byAddress("7");

        if (sensor7 != null)
            sensor7.updateCurrentSensorStatus(decoded.B_phase_current);
    };
 // Store C Phase Current
    if (decoded.C_phase_current != null) {
        var sensor8 = device.endpoints.byAddress("8");

        if (sensor8 != null)
            sensor8.updateCurrentSensorStatus(decoded.C_phase_current);
    };
 ...
 // Store Status
    if (decoded.Equipment_status != null) {
        ...
        if (sensor9 != null)
            sensor9.updateIASSensorStatus(decoded.Equipment_status);
    };
 ...
 // Store Alarm
    if (decoded.Equipment_alarm != null) {
        ...
        if (sensor10 != null)
            sensor10.updateIASSensorStatus(decoded.Equipment_alarm);
    };
